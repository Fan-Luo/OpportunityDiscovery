...
$result_file = 'sorted_by_noisy_or-idf.tsv';
open(OUTPUT, ">$result_file") || die "Cannot open file $result_file for writing\n";
print OUTPUT "predict finding\tgold label\tnoisy_or * log(min(A-IDF,C-IDF))\tgold polarity\tevidence(s)\n";

my %h1=();   # edge_id
my %h2=();   # noisy_or
my %h3=();   # gold label
my %h4=();   # edge lable/polarity (increase or decrease)
my %A_idf=();
my %C_idf=();
...
while (my $line = <$fh>) {
    ...
    if($y_pred){
        ...
        if(exists $h2{$new_finding}){    
            ...
            $A_idf{$new_finding} = $tmp[7];
            $C_idf{$new_finding} = $tmp[8];
        }
    } 
}
...
while( my($new_finding, $pro0_product) = each %h2 ){  
    $noisy_or = 1 - $pro0_product;       #calculate noisy-or
    $h2{$new_finding} = $noisy_or * log(min($A_idf{$new_finding},$C_idf{$new_finding})) ; 
}
